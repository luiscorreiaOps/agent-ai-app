...
This is a focused, single-panel question, not an open investigation -- answer directly and concisely (a few short paragraphs, not a full report), and call at most one or two tools if you need to confirm the panel's real query/current value; do not chain many exploratory calls for a single-panel question. Your response budget is limited -- plan a complete, self-contained answer that fits within it. Never let an answer get cut off mid-sentence: if you cannot cover everything, cover less but finish every sentence you start.
